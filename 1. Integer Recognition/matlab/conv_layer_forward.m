...
for batch = 1:batch_size %do conv on the images
    input_n.data = input.data(:, batch);
    col = im2col_conv(input_n, layer, h_out, w_out);
    col = reshape(col, k * k * c, h_out * w_out); %reshaping the image
    conv_col = zeros([h_out * w_out, num]);
    for pos = 1:(h_out * w_out)
        for f = 1:num
            %do conv on filter area
            conv_col(pos, f) = sum(param.w(:,f) .* col(:,pos)) + param.b(:, f);
        end
    end
    output.data(:, batch) = reshape(conv_col, [h_out * w_out * num, 1]); 
end
end
